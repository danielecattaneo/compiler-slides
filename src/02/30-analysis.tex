...
\begin{frame}{Control Flow Graph}{Instructions are Values}
\vfill
\begin{center}
...
\smallskip
In fact, \cppinline{llvm::Instruction} inherits from \cppinline{llvm::Value}.
\end{center}
\end{block}

\vfill

However, remember that not all instances of\\\cppinline{llvm::Value}
are also \cppinline{llvm::Instruction}s!\\
\smallskip
...
\begin{frame}{Side Note}{Definitions and Uses II}
...
\vfill
There are other kinds of \cppinline{llvm::Value} and \cppinline{llvm::User} than
\cppinline{llvm::Instruction} and \cppinline{llvm::Value}
